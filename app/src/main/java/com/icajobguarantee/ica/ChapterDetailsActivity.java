...
public class ChapterDetailsActivity extends AppCompatActivity implements WebInterface {
    ...
    private void getData(){
        ...
        WebServiceController wc = new WebServiceController(ChapterDetailsActivity.this,ChapterDetailsActivity.this);
            ...
            try {
                ...
                if (jsonArray.length()>0) {
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        listViewId.add(l_id);
                        listViewString.add(l_name + ", " + item_type);
                        listViewCode.add(l_code);
                        listViewTime.add(l_created);
                    ...
                    androidListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                        ...
                        public void onItemClick(AdapterView<?> parent, View view,
                                                final int i, long id) {
                            //Toast.makeText(getActivity(), "GridView Item: " + listViewId.get(+i) + listViewString.get(+i) + listViewImage.get(+i) + listViewCode.get(+i), Toast.LENGTH_LONG).show();
                            Intent intent = new Intent(getApplicationContext(), DetailsActivity.class);

                            intent.putExtra("cid", listViewId.get(+i));
                            intent.putExtra("ctitle", listViewString.get(+i));
                            intent.putExtra("cimage", listViewImage.get(+i));
                            intent.putExtra("ccode", listViewCode.get(+i));
                            intent.putExtra("ctime", listViewTime.get(+i));
                            intent.putExtra("cdetails", listViewDetails.get(+i));
                            intent.putExtra("cvideo", listViewVideo.get(+i));
